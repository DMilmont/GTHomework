...
# Create plot
crimeData %>% 
  gather(key = "iv", value = "x", -Crime, -predicted, -residuals) %>%
  ggplot(aes(x = x, y = Crime)) +
...
modelResiduals %>% 
  ggplot() +
  geom_density() +
  facet_wrap(facets = colnames())

ggplot(modelResiduals, aes(x=values)) + geom_density(aes(group=Normal))
